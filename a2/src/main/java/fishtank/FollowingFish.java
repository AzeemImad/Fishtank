...
public class FollowingFish extends FishTankEntity {
    ...
    public void update() {
        ...
        if(Math.abs(de.getY() - r) > 2) {
            if(de.getY() < r) {
                r -= 1;
                // check if fish is out of bounds (down)
                if (r >= 80){
                    // decrement by 1 so you can bring it back up
                    r -=3;
                }
            }
                ...
                if (de.getY() == this.getY()){
                    if(this.getX() <= 4){
                        // check if the FollowingFish is on the top.
                        r += 2;
                        c += 2;
                    }
                    else if (this.getX() >= 80){
                        // check if the FollowingFish is on the bottom.
                        r -= 2;
                        c -= 2;
                    }
                ...
                r += 1;
                if (r <= 4){
                    // increment by 1 so it goes back up
                    r +=1;
                }
            }
        }
